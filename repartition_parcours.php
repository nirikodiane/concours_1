...
require 'connect.php';
$parcours=$_GET['id'];
$recup=$bdd->prepare('SELECT * FROM candidats WHERE parcours1=? AND centre="Antsiranana" ORDER BY nom');
$recup->execute(array($parcours));
?>
...
<html>
  <head>
    <meta charset="utf-8"/>
    <title>Liste_par_parcours_<?php echo $parcours; ?></title>
  </head>
  <body>
    <style>
    *{
...
<?php
$parcours_req=$bdd->prepare('SELECT * FROM parcours WHERE parcours_abrevie=?');
$parcours_req->execute(array($parcours));
$parcours_utile=$parcours_req->fetch();

if ($parcours_utile['grade_abrevie']=="DTS") {$cycle="cycle1";} else {$cycle="cycle2";}

$date_conc=$bdd->prepare('SELECT * FROM date_concours WHERE Cycle=?');
$date_conc->execute(array($cycle));
$date_concours=$date_conc->fetch();
 ?>

<b><p>
<?php echo $parcours_utile['ecole_complet']; ?> (<?php echo $parcours_utile['ecole_abrevie']; ?>)
<br>Concours d'entrée en
<?php if ($parcours_utile['grade_abrevie']=="DTS") {echo "première année DTS ";} ?>
<?php if ($parcours_utile['grade_abrevie']=="DTSS") {echo " troisième année DTSS ";} ?>
<?php if ($parcours_utile['grade_abrevie']=="ING") {echo "ingéniorat ";} ?>
<?php echo $date_concours["annee_univ"];?>
<br>Session du <?php echo $date_concours["Premier_Date"];?> et <?php echo $date_concours["Deuxiem_Date"];?> <?php echo $date_concours["annee"];?>
<br>Parcours : <?php echo $parcours_utile['parcours_complet']; ?> (<?php echo $parcours;?>)
</p></b>
...
$lieu=$bdd->prepare('SELECT * FROM salle WHERE id_salle=?');
$lieu->execute(array($message['salle']));
$salle=$lieu->fetch();
  ?>
    <tr>
      ...
      <td style="text-align:left;"><?php echo sprintf("%'03d", $message['numero'])."24".$message['parcours1'];?></td>
       ...
       <td style="text-align:auto;width: auto;"><?php echo $salle['lieu_salle'];?></td>
  </tr>
  <?php
  $i++;
}
...
<p style="text-align:center;">Fait à Antsiranana, le <span style="color:black;"> <?php echo date("d/m/Y");?></span></p>
