...
{
  $salle=$_GET['id'];
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE salle=? AND niveau="INGENIORAT" AND ecole=? ORDER BY parcours1, nom');
...
if (isset($_GET['centre']))
{
  ...
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE centre=? AND niveau="INGENIORAT" AND ecole=? ORDER BY parcours1, nom');
...
<html>
  <head>
    <meta charset="utf-8"/>
    <title>Liste_ING_<?php if (isset($salle)) {echo $salle;} if (isset($centre)) {echo $centre;}  echo "_".$_GET['ecole'];?></title>
  </head>
  <body>
    <style>
    *{
...
Liste des candidats régulierements inscrits au concours d'entrée en ingeniorat 2022-2023
