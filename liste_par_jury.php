...
{
  $jury=$_GET['id'];
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE jury=? AND niveau!="DTS" AND ecole=? ORDER BY parcours1, nom');
  $recup->execute(array($jury, $_GET['ecole']));

  $recup1=$bdd->prepare('SELECT * FROM jury WHERE id_jury=?');
  $recup1->execute(array($jury));
...
if (isset($_GET['centre']))
{
  ...
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE centre=? AND niveau!="DTS" AND ecole=? ORDER BY parcours1, nom');
...
$date_conc->execute(array("cycle2"));
...
<html>
  <head>
    <meta charset="utf-8"/>
    <title>Liste_DTSS_<?php if (isset($jury)) {echo $jury;} if (isset($centre)) {echo $centre;}  echo "_".$_GET['ecole'];?></title>
  </head>
  <body>
    <style>
    *{
...
Liste des candidats régulierements inscrits au concours d'entrée en DTSS / Ingéniorat 2022-2023
<br>Session du <?php echo $date_concours["Premier_Date"];?> et <?php echo $date_concours["Deuxiem_Date"];?> 2022
<br><?php if (isset($jury)) {echo "Salle : ".$jury;} if (isset($centre)) {echo "Centre : ".$centre;} ?>
</p>
</b>
...
<br>
  ...
  <br>
<div class="signature">
<p style="text-align:center;">Fait à Antsiranana, le <span style="color:white;">date07/09/2022</span> <?php //echo date("d/m/Y");?></p>
<p style="text-align:center;">Le Directeur Général,</p>
<br>
<p style="text-align:center;"><!--Dr. RAHARIMIHAJA ZAKARIASY Lova--></p>
<br>
</div>
