...
{
  $salle=$_GET['id'];
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE salle=? AND niveau="DTS" AND ecole=? ORDER BY parcours1, nom');
...
<html>
  <head>
    <meta charset="utf-8"/>
    <title>Fiche_de_presence_DTS_<?php if (isset($salle)) {echo $salle;} if (isset($centre)) {echo $centre;}  echo "_".$_GET['ecole'];?></title>
  </head>
  <body>
    <style>
    *{
...
<b><p>
  ...
  Fiche de présence au concours d'entrée en première année DTS <?php echo $date_concours["annee_univ"];?>
