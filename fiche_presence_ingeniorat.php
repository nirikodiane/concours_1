...
{
  $salle=$_GET['id'];
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE salle=? AND niveau="INGENIORAT" AND ecole=? ORDER BY parcours1');
...
if (isset($_GET['centre']))
{
  ...
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE centre=? AND niveau="INGENIORAT" AND ecole=? ORDER BY parcours1');
...
<html>
  <head>
    <meta charset="utf-8"/>
    <title>Fiche_de_presence_ING_<?php if (isset($salle)) {echo $salle;} if (isset($centre)) {echo $centre;}  echo "_".$_GET['ecole'];?></title>
  </head>
  <body>
    <style>
    *{
...
<b><p>
  ...
  Fiche de présence au concours d'entrée en formation d'ingénieur <?php echo $date_concours["annee_univ"];?>
<br>Session du <?php echo $date_concours["Premier_Date"];?> et <?php echo $date_concours["Deuxiem_Date"]." ".$date_concours["annee"];?>
