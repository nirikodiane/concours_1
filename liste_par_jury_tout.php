...
{
  $jury=$_GET['id'];
  $recup=$bdd->prepare('SELECT * FROM candidats WHERE jury=? AND niveau!="DTS" AND centre=? ORDER BY parcours1, nom');
  $recup->execute(array($jury, "Antsiranana"));
...
<html>
  <head>
    <meta charset="utf-8"/>
    <title>Liste_<?php if (isset($jury)) {echo $jury;}?></title>
  </head>
  <body>
    <style>
    *{
...
<b><p>
Liste des candidats régulierements inscrits au concours d'entrée en DTSS / Ingéniorat 2024-2025
<br>Session du <?php echo $date_concours["Premier_Date"];?> et <?php echo $date_concours["Deuxiem_Date"];?> 2024
<br><?php if (isset($jury)) {echo "Salle : ".$jury;} ?>
</p>
</b>
